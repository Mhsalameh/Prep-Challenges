...
const cvsFiltration = (arr) => {
    // write your code here
    let finalResult=[]
   let result= arr.filter (obj => obj.yearsOfExperience > 4 && obj.tech=="JS");
    
    result.forEach(element => {
        let resultObj={ 
            fullName: (element.LastName != null && element.firstName != null) ? `${element.firstName} ${element.LastName}` : (element.firstName == null) ? `${element.LastName}` : `${element.firstName}`,
             tech : element.tech,
        }
...
const vowelsFiltration = (arr) => {
    ...
    let  result= arr.filter(word => vowel.test(word)? true : word=='dry'? true : false);
    return result;
} 
