...
function fullName(arr) {
    // write your code here
    let result=[];
    result=arr.map(obj => `${obj.firstName} ${obj.lastName}`);
    return result;
}
...
function gradesAvg(arr) {
    function avg(arr){
        ...
        let averag=sum/arr.length;
        return averag
    }
    arr.map(obj => obj.avg= avg(obj.gradsList))
...
function studentsResult(arr) {
    // write your code here
    arr.map(obj => obj.result= (obj.avg>=50) ? "Passed" : "Failed");
    return arr;
}
